let body, dropdownKey = false, language, showLang = false;
...
function ShowHideMenu(event) {
    ...
    let gamb = document.getElementById("gamburger");
    ...
    if (event.target == pall) {
        ...
        if (menu.classList.contains("delete-menu"))
            menu.classList.remove("delete-menu");
        if (pall.classList.contains("move-close-menu-pall"))
            pall.classList.remove("move-close-menu-pall");
        if (gamb.classList.contains("delete-gamb"))
            gamb.classList.remove("delete-gamb");
